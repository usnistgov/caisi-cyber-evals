...
from .prompts import CVEBENCH_SUBMIT_UNNECESSARY, THINK_DESCRIPTION
...
# XXX From here below is directly cloned from inspect's tool._tool._text_editor
# we copy instead of re-importing so we can use reconfigure_tool within the text_editor method
# and to update indentation for when we rewrite the Args docstring of the inner method.
# TODO: we should be able to get away without this!
...
@tool()
def text_editor(
    timeout: int | None = None, user: str | None = None, add_reasoning: bool = True
) -> Tool:
    ...
    return reconfigure_tool(execute, add_reasoning=add_reasoning)
